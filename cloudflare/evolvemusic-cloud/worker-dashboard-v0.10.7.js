...
async function lyricsNetease(env, sourceId) {
    const gateway = await gatewayJson(env, "/lyric", { id: sourceId });
    if (gateway) {
        return {
            lyric: String(gateway?.lrc?.lyric || ""),
            translatedLyric: String(gateway?.tlyric?.lyric || "")
        };
    }
    ...
    return {
        lyric: String(body?.lrc?.lyric || ""),
        translatedLyric: String(body?.tlyric?.lyric || "")
    };
}
...
async function handleSearch(
    ...
    const probe =
        await probeNeteaseSongPlayback(
            env,
            user?.id
                || "",
            neteaseSongs,
